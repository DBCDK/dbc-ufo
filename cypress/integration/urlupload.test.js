...
context('Testing url upload component', () => {
  ...
  it('Should render component', () => {
    ...
    cy.get('.url-upload textarea').should(
      'have.attr',
      'placeholder',
      'Skriv en URL pr. linie'
    );
  });

  it('Should show preview of image from url', () => {
    cy.addUrl(
      'https://www.colourbox.dk/preview/2582621-white-horses-grazing-on-ranch.jpg'
    );
    cy.get('.preview-images img')
      .invoke('attr', 'alt')
      .should('include', 'horses-grazing-on-ranch.jpg');
  });
  ...
  it('Should reject non image', () => {
    ...
    cy.get('.preview.error .preview-error--content .message', {
      timeout: 30000
    }).should('contain', 'Fejl');
    cy.get('.preview.error .preview-error--content .message').should(
      'contain',
      'Ugyldig billedurl'
    );
  });
